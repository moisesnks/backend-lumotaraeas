...
async function asignarResponsables(id, responsables) {
    ...
    try {
        await db.runTransaction(async (transaction) => {
            ...
            responsables.forEach(uid => responsablesActuales.add(uid));
            const responsablesFinales = Array.from(responsablesActuales);

            // Actualizar la tarea con los nuevos responsables
        ...
        console.log('Responsables y tareas asignadas exitosamente');
    } catch (error) {
        console.error('Error al asignar responsables y tareas:', error);
        throw error;
    }
}

module.exports = asignarResponsables;
